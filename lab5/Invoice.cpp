//
// contains definitions for the Invoice class.
// This class is responsible for printing an invoice
// to the user and determining the price of the parking pass,
// taking into account discounts or extra chares based on the 
// user's input.
//
...
void Invoice::generalPersonPrinter() 
// will print name, email, address, state--things that apply to everyone
{
...
void Invoice::constructionPrinter(const Construction& construction) const 
// print attributes unique to construction workers
{
...
void Invoice::generalVehiclePrinter()  // print attributes shared by all vehicles
{
...
// will call necessary getters from necessary classes to determine final price of the parking permit
{
    ...
    if (PERSON_TYPE == 1) //student
        {
            ...
            if (student.getState() == "Nevada" || student.getState() == "nevada" 
                || student.getState() == "NV")
                {PRICE -= 15;} // discount for being a NV native
            if (student.getMajor() == "Physics" || student.getState() == "physics" 
                || student.getState() == "PHYS")
                {PRICE -= 15;} // discount for being a physics major
        }
    ...
    if (VEHICLE_TYPE == 4)// scooter
        {
            PRICE += 10000; // exorbitant charge for having a scooter
        }
